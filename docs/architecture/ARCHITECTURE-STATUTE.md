...
│   ├── auth/                # 인증 페이지 (/auth — 로그인/회원가입 탭 전환)
│   │   ├── page.tsx         # Server Component (인증 체크 + 레이아웃)
│   │   ├── AuthTabs.tsx     # Client Component (탭 전환)
...
└── types/
    └── database.ts          # DB 타입 (profiles, beans, cupping_notes, bean_ratings, likes, comments)
```
