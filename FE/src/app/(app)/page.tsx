...
export default async function HomePage() {
  ...
  return (
    <main className="mx-auto flex max-w-md flex-col gap-6 px-4 py-8">
      <section className="flex flex-col gap-2 rounded-2xl bg-[#f7f1eb] px-5 py-6">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#8B2635]">
          Public Feed
        </p>
        <div className="flex items-end justify-between gap-4">
          <div className="flex flex-col gap-1">
            <h1 className="text-2xl font-bold text-gray-900">최신 커핑 피드</h1>
            <p className="text-sm text-gray-600">
              회원이 아니어도 최근 기록을 둘러보고, 마음에 드는 노트는 상세 페이지에서 이어서 볼 수 있어요.
            </p>
          </div>
          <Link href="/cupping" className="shrink-0 text-sm font-semibold text-[#8B2635]">
            필터해서 보기
          </Link>
        </div>
      </section>
